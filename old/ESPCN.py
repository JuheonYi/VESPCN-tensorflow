...
class ESPCN(object):
    ...
    def train(self, config, load = True):
        ...
        for epoch in range(self.config.epoch):
            ...
            for idx in range(0, batch_idxs):
                ...
                batch_HR = np.array(batch)#.astype(np.float32) #float32로 변환 안해도 알아서 변환되는 듯...
    ...
    def test(self, name = "Set5", load = True):
        ...
        #img_list = sorted(glob(os.path.join("/home/johnyi/deeplearning/research/VSR_Datasets/test/vid4/",str(name),"*.png")))
        ...
        for i in range(0,len(img_list)):
            ...
            if self.mode == "RGB":
                ...
                #out = np.round(((1+np.tanh(out.eval())[0])*(235-16)/2) + 16)
                #180325
    ...
    def save(self, checkpoint_dir):
        model_name = "ESPCN-"+str(self.mode)+"-x"+str(self.scale)
    ...
    def load(self, checkpoint_dir):
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            #self.saver.restore(self.sess, "/home/johnyi/deeplearning/research/CVPR16-ESPCN-TF-jhyi16/checkpoint/VSR/DIV2K/VESPCN-RGB-x3")
            return True
        else:
            return False